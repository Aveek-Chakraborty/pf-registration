...
const WalkCrossComponent: React.FC = () => {
  const [data, setData] = useState<Walkcrossdata[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/topwalks');
      if (!response.ok) {
  ...
  const generatePdf = async (data: Walkcrossdata[]) => {
    const response = await fetch('/api/topwalks');
    if (!response.ok) {
      throw new Error('Failed to fetch data');
    }
  ...
  const generatePDF = async (data: Walkcrossdata[]) => {
    ...
    try {
      ...
      link.href = url;
      link.download = 'walkcross_data.pdf';
      link.click();
  ...
  return (
    <div className="bg-gray-100 p-4 rounded-lg shadow-md relative m-3 w-1/2 ">
      <div className='flex justify-between'>
        <h1 className="text-xl font-bold mb-4">Walkcross Data</h1>
        {showData && (
